<script lang="ts">
    import {fly} from 'svelte/transition'
    import ProfileEntry from "$lib/components/profile/entry.svelte";
    import ProfileAvatar from "$lib/components/profile/avatar.svelte";
    import {AvatarSourceUrl} from "$lib/assets/avatar-source";
    import MyButton from '$lib/components/my-button.svelte';
    import {GlobalVars} from "$lib/global-accessor";
    import ModalHost from '$lib/components/modals/modal-host.svelte';

    import AboutWebsite from './about-website.svelte';
    import NameCard from '$lib/components/name-card.svelte';
    import type { SnsEntryReminderModalText } from '$lib/components/text-parts';

    function about() {
        const modalHost = GlobalVars.get("ModalHost") as ModalHost;
        modalHost.showModal(AboutWebsite, undefined, {header: "关于"});
    }

    const reminderModalTexts: SnsEntryReminderModalText = {
      header: "提示",
      proceed: "点此继续",
      loading: "传送中",
      cancel: "取消"
    }
</script>

<svelte:head>
    <title>Portfolio</title>
    <meta name="description" content="我的极简简历 🫠"/>
    <meta property="og:description" content="我的简历和在线联系方式"/>
    <meta property="og:title" content="Portfolio"/>
    <meta property="og:type" content="profile"/>
    <meta property="og:profile:username" content="appleneko2001"/>
    <meta property="og:image" content={AvatarSourceUrl}/>
</svelte:head>

<div class="profile-page"
    in:fly={{ y: -100, duration: 300 }}
    out:fly={{ y: -100, duration: 300 }}>
    <ProfileAvatar src={AvatarSourceUrl}/>

    <NameCard name="笨蛋夜星" username="appleneko2001"/>

    <br/>

    <div class="entries">
        <ProfileEntry icon="x" text="X"
                      showLoading={true}
                      secretIcon="https://cdn.jsdelivr.net/npm/simple-icons@v10.0.0/icons/twitter.svg"
                      secretText="推特"
                      href="https://twitter.com/appleneko2001"
                      modalTexts={reminderModalTexts}/>
        <ProfileEntry icon="discord" text="Discord" showLoading={true}
                      modalTexts={reminderModalTexts}
                      href="https://discord.com/users/499588722174459914">
            <style>
                .reminder {
                    margin: 8px;
                    max-width: 320px;
                    text-align: justify;
                }
            </style>
            <div class="reminder">
                因近期 discord 聊天诈骗机器人较为猖狂，请先用其他方式预先告知我后再添加联系方式喔！
                <br/>
                感谢您的理解 🙏
            </div>
        </ProfileEntry>
        <ProfileEntry icon="bluesky" text="蓝天" showLoading={true}
                      href="https://appleneko2001.bsky.social"
                      modalTexts={reminderModalTexts}/>
        <ProfileEntry icon="osu" text="osu!" showLoading={true}
                      href="https://osu.ppy.sh/users/11455502"
                      modalTexts={reminderModalTexts}/>
    </div>

    <div class="entries">
        <ProfileEntry icon="codeberg" text="Codeberg" showLoading={true}
                      href="https://codeberg.org/appleneko2001"
                      modalTexts={reminderModalTexts}/>
        <ProfileEntry icon="github" text="GitHub" showLoading={true}
                      href="https://github.com/appleneko2001"
                      modalTexts={reminderModalTexts}/>
    </div>


    <!-- <div class="entries">
        <ProfileEntry icon="material:folder_special" text="Works" expanded={true} hint={null}
                      href="/works"/>
    </div> -->

    <div class="footer-area">
        <MyButton text="关于" icon={{ icon: 'material:info', size: 16 }} click={about}/>
        <!--<MyButton text="Link exchange" icon={{ icon: 'material:badge', size: 16 }} click={exchangeLinks}/>-->
    </div>
</div>

<style>
    .profile-page {
        /* Fallback for legacy browsers */
        min-height: max(600px, calc(100vh - 80px));
        min-height: max(600px, 100dvh);
        display: flex;
        flex-direction: column;
        align-items: center;
        justify-content: center;
        gap: 16px;
    }

    .entries {
        display: flex;
        justify-content: center;
        flex-flow: wrap;
        align-content: center;
        gap: 0.5rem;
    }

    .footer-area {
        position: absolute;
        bottom: 0;
        margin: 16px 8px;
        display: flex;
    }

    @media screen and (width < 600px) {
        .entries {
            max-width: 390px;
        }
    }
</style>
